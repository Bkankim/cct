...
#   CCT_DISABLE_WEB_FEATURES=0 → 라벨 실행에서도 Advisor/텔레메트리 등 비필수 웹 호출 허용 (기본 1=차단, 자동업데이트는 유지)
...
#   CCT_GJC_WARN=0       → gjc(가재코드) 저장 자격증명 잔존 경고 끄기
#
...
_cct_add() {
  ...
  if [ "${CCT_STICKY:-1}" != "0" ] && [ "$(_cct_active_label)" = "$label" ]; then
    ...
    if [ "${CCT_DISABLE_WEB_FEATURES:-1}" = "0" ]; then
      ...
      case "${CLAUDE_CODE_DISABLE_ADVISOR_TOOL+x}${CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC+x}${CLAUDE_CODE_DISABLE_BACKGROUND_PLUGIN_REFRESH+x}${DISABLE_TELEMETRY+x}${DISABLE_ERROR_REPORTING+x}${DISABLE_BUG_COMMAND+x}${DISABLE_FEEDBACK_COMMAND+x}" in
        "") ;;
        *) return 1 ;;
      esac
      /usr/bin/env | /usr/bin/grep -q '^CLAUDE_CODE_OAUTH_TOKEN=' || return 1
    else
      # 상속된 NONESSENTIAL 을 지워 자식 claude 자동업데이트 차단 누수를 막고(존재하면 안 됨),
      # 부재는 opt-out 분기와 같은 ${VAR+x} 가드로 검증한다.
      ...
      case "${CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC+x}" in
        "") ;;
        *) return 1 ;;
      esac
...
_cct_apply_env() {  # $1=token — 현재 셸에 토큰(+웹기능 차단 플래그) export
  ...
  if [ "${CCT_DISABLE_WEB_FEATURES:-1}" = "0" ]; then
    ...
    # 구버전 cct 가 sticky 로 상속시킨 NONESSENTIAL 을 unset. 안 지우면 자식 claude 로
    # 새어 자동업데이트가 계속 꺼진다. (DISABLE_AUTOUPDATER 는 사용자 탈출구라 건드리지 않음)
    unset CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC
    export CLAUDE_CODE_DISABLE_ADVISOR_TOOL=1 DISABLE_TELEMETRY=1 DISABLE_ERROR_REPORTING=1 DISABLE_BUG_COMMAND=1 DISABLE_FEEDBACK_COMMAND=1 CLAUDE_CODE_DISABLE_BACKGROUND_PLUGIN_REFRESH=1
  fi
}
  ...
  *[![:space:]]*)
  ...
  if [ "${CCT_STICKY:-1}" = "0" ]; then
    if [ "${CCT_DISABLE_WEB_FEATURES:-1}" = "0" ]; then
      ...
      # 상속된 NONESSENTIAL 을 unset(자식 자동업데이트 차단 누수 방지)하고 NEW+ADVISOR/REFRESH 를
      # export 한 뒤 기존과 동일하게 claude 를 호출한다. 셸/디스크는 그대로(서브셸 국한).
      (
        unset CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC
        export CLAUDE_CODE_DISABLE_ADVISOR_TOOL=1 DISABLE_TELEMETRY=1 DISABLE_ERROR_REPORTING=1 DISABLE_BUG_COMMAND=1 DISABLE_FEEDBACK_COMMAND=1 CLAUDE_CODE_DISABLE_BACKGROUND_PLUGIN_REFRESH=1
        CLAUDE_CODE_OAUTH_TOKEN="$tok" ANTHROPIC_OAUTH_TOKEN="$tok" "${command_args[@]}" "$@"
