...
| 영역 | 내용 | 비용 |
|---|---|---|
| 지갑 상태 | `cct status` + `cct doctor` 요약 | 프로브 없음 |
| 추천 배너 | 가장 빡빡한 창(5h/7d/7f) 기준으로 갈아탈 계정 1개 | 프로브 없음 |
| 활성 세션 라이브 | statusline 캐시의 rate_limits, 모델, 컨텍스트, 세션 비용 | 프로브 없음 |
| 계정 카드 | 라벨별 5h/7d/7f 게이지·리셋 카운트다운·status 플래그·org·check | 갱신·점검 시 프로브 |
| 리셋 타임라인 | 다음 24시간의 창 리셋 시점 | 프로브 없음 |
| doctor | PASS / WARN / FAIL | 프로브 없음 |
| 실행 로그 | 서버가 실행한 cct 명령과 rc, 소요 시간 | 프로브 없음 |
